...
namespace ccxc_backend.Controllers.Admin
{
    ...
    public class OracleAdminController : HttpController
    {
        ...
        public async Task QueryOracle(Request request, Response response)
        {
            ...
            List<oracle> resultList;
            if (requestJson.gid == 0)
            {
                if (requestJson.reply == 0)
                {
                ...
                {
                    resultList = await messageDb.SimpleDb.AsQueryable().Where(it => it.is_reply == replyType)
                        .OrderBy(it => it.create_time, orderType).ToPageListAsync(page, pageSize, totalCount);
                }
            }
            else
            {
                ...
                {
                    resultList = await messageDb.SimpleDb.AsQueryable().Where(it => it.is_reply == replyType && it.gid == requestJson.gid)
                        .OrderBy(it => it.create_time, orderType).ToPageListAsync(page, pageSize, totalCount);
                }
            }
            ...
            var puzzleDict = (await puzzleDb.SelectAllFromCache()).ToDictionary(it => it.second_key, it => it);
